...
from Data_Loader_600 import get_test_loader
...
def evaluate_model(model, test_loader, device):
    ...
    with torch.no_grad():  # Disable gradient computation
        for images, targets in test_loader:
            ...
            outputs = [{k: v.to(device) for k, v in t.items()} for t in outputs]  # Move outputs to CPU
...
if __name__ == '__main__':
    ...
    test_loader = get_test_loader(num_workers=0)  # Ensure num_workers=0
    ...
    model.load_state_dict(torch.load("best_model_cpu_600.pth", map_location=device))
