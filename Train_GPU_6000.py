...
def train_model(model, train_loader, val_loader, device, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        if val_map > best_map:
            best_map = val_map
            torch.save(model.state_dict(), "best_model_gpu_6000.pth")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('gpu_6000_loss_curve.png')  # Save the plot as an image file
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('gpu_6000_map_curve.png')  # Save the plot as an image file
    plt.show()
...
if __name__ == '__main__':
    ...
    train_loader, val_loader = get_data_loaders(batch_size_train=8, batch_size_val=16, num_workers=4)
