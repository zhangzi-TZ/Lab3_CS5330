...
from Data_Loader_6000 import get_data_loaders
...
def train_model(model, train_loader, val_loader, device, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, targets) in enumerate(train_loader):
            ...
            # Print loss for each batch every 10 batches
            if batch_idx % 10 == 0:
        ...
        if val_map > best_map:
            best_map = val_map
            torch.save(model.state_dict(), "best_model_cpu_6000.pth")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('cpu_6000_loss_curve.png')  # Save the plot as an image file
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('cpu_6000_map_curve.png')  # Save the plot as an image file
    plt.show()
...
if __name__ == '__main__':
    ...
    # Get data loaders
    train_loader, val_loader = get_data_loaders(batch_size_train=4, batch_size_val=8,
                                               num_workers=0)  # Set num_workers=0 for CPU

    # Load model and move to device (CPU)
    model = get_model()
    model.to(device)
