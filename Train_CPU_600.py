...
def train_model(model, train_loader, val_loader, device, num_epochs=10):
    """
    Train the Faster R-CNN model with validation and plot the loss curve.
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Starting epoch {epoch + 1}/{num_epochs}...")  # Print epoch start
        ...
        if val_map > best_map:
            best_map = val_map
            torch.save(model.state_dict(), "best_model_cpu_600.pth")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('map_curve.png')  # Save the plot as an image file
    plt.show()
...
if __name__ == '__main__':
    ...
    # Get data loaders
    train_loader, val_loader = get_data_loaders(num_workers=0)  # Set num_workers=0 for CPU
